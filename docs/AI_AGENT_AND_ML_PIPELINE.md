...
Данный документ является логическим продолжением **[SYSTEM_SPECIFICATION_AND_PRODUCT_GUIDE.md](./SYSTEM_SPECIFICATION_AND_PRODUCT_GUIDE.md)** (бизнес-контекст, FR/NFR) и **[ARCHITECTURE_AND_DATA_MODEL.md](./ARCHITECTURE_AND_DATA_MODEL.md)** (архитектурный фундамент, модель данных). Для детального ознакомления с голосовым пайплайном следует обратиться к **VOICE_PIPELINE.md**, а для API-контрактов агентов — к **API_REFERENCE.md**.
